...
> **本文档定位**：本文件是 `mqtt-server-cluster.md` 的**路由层专章**，专注于跨节点 PUBLISH 消息路由与 MQTT 5.0 共享订阅（`$share` / `$queue`）的去重投递问题。配套文档：
> - `docs/todo/mqtt-server-cluster.md`（基础集群）
> - `docs/todo/mqtt-server-cluster-storage.md`（存储层，可选依赖）
...
路由层不依赖存储层，但存储层能为路由层带来：

- Shared subscription 状态持久化（H2）
- 节点宕机后路由表快速恢复（L1 从 L2 加载）
...
| 方案 | 转发次数 | 投递次数 | 复杂度 | 单点 | 状态 |
|---|---|---|---|---|---|
| V1 全量广播 | N | N | 低 | 无 | 内存 |
| V2.1 owner 选举 | 2 | 1 | 中 | **owner** | 内存 |
| V2.2 主备 | 2 | 1 | 高 | 容忍 1 | 内存 |
| **V3 dispatcher** | **1** | **1** | **中** | **无** | **内存** |
| V4 dispatcher + H2 | 1 | 1 | 中 | 无 | **持久化** |
...
```
V1 (现状)            -> V3 (本文)
  阶段 A: 仅加 strategy 接口, dispatcher 仍用 V1 广播逻辑
  阶段 B: dispatcher 切换到 dispatcher 模型
  阶段 C: 加 storage 层 (H2)
```

**建议**：阶段 A 和 B 可以平滑升级，向后兼容。V1 节点在升级期间会重复投递，但不影响功能正确性（重复 = 多发，业务层去重）。
...
- [ ] 共享订阅状态持久化（H2，可选）— 详见 `mqtt-server-cluster-storage.md` §4.4
- [ ] 节点重启后策略计数器恢复（可选）
...
**配套文档**：
- `docs/todo/mqtt-server-cluster.md`（基础集群）
- `docs/todo/mqtt-server-cluster-storage.md`（存储层）
