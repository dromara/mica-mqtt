...
>
> **配套文档**（按演进路线阅读，版本已对齐）：
> - `mqtt-server-cluster.md` (v2.8) — 基础集群、V1 已实现能力
> - `mqtt-server-cluster-storage.md` (v1.1) — V3 持久化层（H2 MVStore 统一引擎）
> - **本文档** (v1.1) — V2 路由层（EMQX dispatcher 风格）+ 共享订阅
...
### 1.5 与 cluster v2.8 / storage v1.1 的协议消息对齐

本文档新增的 3 个协议消息已在 cluster v2.8 的 `ClusterMessageType` 枚举中注册：

| 枚举值 | 名称 | 方向 | 用途 |
|---|---|---|---|
| 9 | `SHARED_SUBSCRIBE_FORWARD` | 任意 → 任意 | 共享订阅跨节点注册 |
| 10 | `SHARED_PUBLISH_FORWARD` | publisher → dispatcher | publisher 转发给 dispatcher（可选优化） |
| 11 | `SHARED_DISPATCH_TO_CLIENT` | dispatcher → 目标节点 | **dispatcher 模型核心**，本地决策后只发给 1 个目标节点 |

storage 文档（v1.1）新增的 8 个协议消息（12-19）与本文档**无功能重叠**，可独立实现。
...
└── cluster/
    ...
    │   ├── SharedSubscribeNotifyMessage.java    # [新增] 共享订阅通知
    │   ├── SharedDispatchToClientMessage.java   # [新增] dispatcher -> 目标节点
    │   └── SharedPublishFanoutMessage.java      # [新增] publisher -> dispatcher (可选优化)
    └── pipeline/
        ├── ClusterMessageDispatcher.java        # [重写] dispatcher 模型
        └── strategy/                            # [新增] 策略目录
...
private void deliverShared(String topic, MqttPublishMessage msg) {
    ...
    for (var entry : byGroup.entrySet()) {
        Subscribe picked = strategy.pick(entry.getValue(), msg, entry.getKey());
...
public interface SharedSubscriptionStrategy {
     ...
     * @param msg         当前消息 (用于 hash / packetId 等)
     * @param groupName   共享订阅组名 (用于 group 级状态)
     * @return 选中的订阅者; 返回 null 表示 group 当前无活跃订阅者
     */
    Subscribe pick(List<Subscribe> candidates, MqttPublishMessage msg, String groupName);
...
public class RandomStrategy implements SharedSubscriptionStrategy {
    ...
    public Subscribe pick(List<Subscribe> candidates, MqttPublishMessage msg, String group) {
...
public class RoundRobinStrategy implements SharedSubscriptionStrategy {
    ...
    public Subscribe pick(List<Subscribe> candidates, MqttPublishMessage msg, String group) {
        if (candidates.isEmpty()) return null;
        long seq = counters.computeIfAbsent(group, k -> new AtomicLong(0)).getAndIncrement();
...
public class LocalFirstStrategy implements SharedSubscriptionStrategy {
    ...
    public Subscribe pick(List<Subscribe> candidates, MqttPublishMessage msg, String group) {
        ...
        List<Subscribe> locals = candidates.stream()
            .filter(s -> clientNodeMap.get(s.getClientId()).equals(myNodeId))
            .collect(Collectors.toList());

        if (!locals.isEmpty()) {
            return locals.get(rnd.nextInt(locals.size()));
        }
...
public class HashClientStrategy implements SharedSubscriptionStrategy {
    public Subscribe pick(List<Subscribe> candidates, MqttPublishMessage msg, String group) {
        ...
        int hash = Math.abs(msg.getPacketId() ^ group.hashCode());
...
public class StickyStrategy implements SharedSubscriptionStrategy {
    ...
    public Subscribe pick(List<Subscribe> candidates, MqttPublishMessage msg, String group) {
        ...
        sticky.put(group, picked.getClientId());
        return picked;
    }
...
public enum ClusterMessageType {
    ...
    // routing 文档新增
    SHARED_DISPATCH_TO_CLIENT(9),     // dispatcher -> 选中节点
    SHARED_SUBSCRIBE_NOTIFY(10),      // 共享订阅变更广播 (区别于普通 SUBSCRIBE_NOTIFY)
    SHARED_SUBSCRIBE_REMOVE(11);      // 共享订阅取消广播
}
```

> **注**：`SharedSubscribeNotify` 在 V1 中可能复用 `SUBSCRIBE_NOTIFY`，但为清晰起见本文档建议**独立消息类型**，便于未来按 group 单独过滤。
...
MqttServer server = MqttBroker.create()
    ...
    .clusterConfig(new MqttClusterConfig()
        .enabled(true)
        .sharedSubStrategy(new LocalFirstStrategy(clientNodeMap, myNodeId))
        // .sharedSubStrategy(new RoundRobinStrategy())
        // .sharedSubStrategy(new HashClientStrategy())
        .seedMembers(...)
    )
    .start();
```
...
**v1.1 变更**：与 cluster v2.8 / storage v1.1 对齐，演进路径分三档：
...
| **C** | 引入 H2 持久化（参见 storage v1.1） | 兼容 V1/V2 | 1 周（与 storage 阶段 0-2 同步） |

**建议**：阶段 A 和 B 可以平滑升级，向后兼容。V1 节点在升级期间会重复投递共享订阅消息，但不影响功能正确性（业务层去重）。阶段 C 涉及存储层，建议作为独立项目按 storage v1.1 实施路径推进。
...
- [x] `SHARED_DISPATCH_TO_CLIENT` (9) — dispatcher 模型核心
- [x] `SHARED_SUBSCRIBE_NOTIFY` (10) — 可选拆分
- [x] `SHARED_SUBSCRIBE_REMOVE` (11) — 可选拆分
...
- [ ] 共享订阅 owner 状态持久化（H2 `mqtt_shared_sub` 表）— 详见 `mqtt-server-cluster-storage.md` (v1.1) §4.4
- [ ] Shared Sub 故障切换：owner 宕机时 backup 升级（storage v1.1 §4.4.4，零消息真空）
...
**v1.1 变更摘要**（与 cluster v2.8 / storage v1.1 同步）：
- 顶部配套文档导航：版本号统一标注
- §1.3 重写：明确与 storage 文档的依赖关系（路由不依赖存储）
- §1.4 新增：V1/V2/V3 版本路线图对齐表
- §1.5 新增：与 cluster v2.8 / storage v1.1 的协议消息对齐说明
- §5 对比表：原"V3 dispatcher"改为"V2 dispatcher"（与 cluster v2.8 演进分层对齐），"V4 dispatcher + H2"改为"V2 dispatcher + V3 持久化"
- §8.3 升级路径：分 V1 → V1.5 → V2 → V2+V3 四档，标注各阶段兼容性
- §9.4 与 storage 协同：更新为 storage v1.1 方案（H2 `mqtt_shared_sub` 表 + 故障切换）

**配套文档**：
- `docs/todo/mqtt-server-cluster.md` (v2.8)（基础集群）
- `docs/todo/mqtt-server-cluster-storage.md` (v1.1)（存储层）
