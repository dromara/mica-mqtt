...
└── cluster/
    ...
    │   ├── SharedDispatchToClientMessage.java # [V2] dispatcher -> 目标节点 (code=11)
    ...
    │       ├── LocalFirstStrategy.java
    │       ├── HashClientStrategy.java
    │       └── StickyStrategy.java
    ├── metrics/                           # 监控指标
    │   └── ClusterMetrics.java            # 集群指标采集
    └── store/                             # V3 存储层
        ├── LocalKvStore.java              # 存储抽象接口
        ├── MemoryKvStoreImpl.java         # 内存实现（测试用）
        ├── H2MvStoreImpl.java             # H2 MVStore 实现
        ├── SessionStore.java              # Session 存储抽象
        ├── H2SessionStore.java            # Session H2 实现
        ├── SharedSubStore.java            # 共享订阅存储抽象
        ├── H2SharedSubStore.java          # 共享订阅 H2 实现
        ├── InflightStore.java             # 飞行消息存储抽象
        ├── H2InflightStore.java           # 飞行消息 H2 实现
...
public class ClusterMessageDispatcher extends BaseMessageHandler {
    ...
    public boolean handle(Message message) {
        // V2 dispatcher 模型：
        //   1. 分离普通订阅 vs 共享订阅
        //   2. 普通订阅: 按节点分组，每个远程节点发一次 PublishForwardMessage
        //   3. 共享订阅: 每个 group 用 strategy 选 1 个 subscriber，
        //      本地则直接投递，远程则发 SharedDispatchToClientMessage
        //   4. 返回 true 继续本地分发
    }
}
```

**V2 已实现**：dispatcher 已改造为 EMQX dispatcher 模型，发布者所在节点本地决策“只发给 1 个目标节点”，消除 V1 的重复转发问题。新增协议消息 `SHARED_DISPATCH_TO_CLIENT(11)`。接收端由 `MqttClusterManager.handleSharedDispatchToClient()` 处理，含 plan-B 重选逻辑。
...
- [x] V1: 共享订阅（`$share` / `$queue`）基础支持
- [x] V2: 共享订阅 dispatcher 模型（EMQX 风格）— `SHARED_DISPATCH_TO_CLIENT(11)` 协议，已实现 5 种策略
