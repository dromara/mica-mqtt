...
| `mqtt_cluster_members` | gauge | 当前已发现的存活节点数（含本节点） |
| `mqtt_cluster_direct_members` | gauge | 当前存在直连 DATA 通道的节点数（含本节点） |
| `mqtt_cluster_topology_healthy` | gauge | 直连节点是否覆盖全部已发现节点；必须为 1 |
...
2. 至少配置一个当前在线 seed，按滚动顺序加入；集群 DATA 不经中间节点转发，最终必须形成两两直连，不能停留在 bootstrap 星型拓扑。
3. 确认 `storage_healthy=1`、`topology_healthy=1` 且 `direct_members == members`；任一节点不满足都不得加入负载均衡。
...
- 通过 `-Pcluster-chaos` 运行 3/5 个独立 JVM 的强杀、成员收缩、同端口重启和双向广播恢复验收。
- 通过 `-Pcluster-integration` 运行 3 节点真实 MQTT QoS 1/2、共享订阅去重与跨节点迁移验收。
