 ...
 * @author L.cm
 */
public class MqttPubAckHandler extends AbstractMqttMessageHandler {
	private static final Logger logger = LoggerFactory.getLogger(MqttPubAckHandler.class);

	private final IMqttSessionManager sessionManager;

	public MqttPubAckHandler(MqttServerCreator serverCreator,
	...
	public MqttMessageType[] messageTypes() {
		return new MqttMessageType[]{MqttMessageType.PUBACK};
	}
	...
	public void handle(ChannelContext context, MqttMessage rawMessage) {
		...
		logger.debug("PubAck - clientId:{} packetId:{} reasonCode:0x{}", clientId, packetId, Integer.toHexString(reasonCode & 0xFF));
		...
		if (reasonCode != MqttPubAckReasonCode.SUCCESS.value()) {
			logger.warn("PubAck failure - clientId:{} packetId:{} reasonCode:0x{}", clientId, packetId, Integer.toHexString(reasonCode & 0xFF));
		}
	...
	private byte getReasonCode(MqttMessageIdVariableHeader variableHeader) {
		...
		return MqttPubAckReasonCode.SUCCESS.value();
	}
}
